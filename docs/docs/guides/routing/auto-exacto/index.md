---
source_url: "https://openrouter.ai/docs/guides/routing/auto-exacto"
title: "Auto Exacto | Smarter provider routing for tool-calling requests | OpenRouter | Documentation"
crawled_at: "2026-04-08T10:29:41.816398+00:00"
---
...
Without Auto Exacto, OpenRouter’s default routing is primarily [price-weighted](../provider-selection/index.md) — requests are load balanced across providers with a strong preference for lower cost. Auto Exacto changes this for tool-calling requests by reordering providers based on quality signals instead of price.
...
- **`provider.sort` parameter** — set `sort` to `"price"` in the `provider` object of your request body. See [Provider Sorting](../provider-selection/index.md) for details.
- **`:floor` virtual variant** — append `:floor` to any model slug (e.g. `openai/gpt-4o:floor`) to sort by price. See [Floor Price Shortcut](../provider-selection/index.md).
